...
def login():
    # Voter Login Page
    if request.method=='POST':
        #create a query and check in database for name , password , mobile number
        name=request.form['name']
        password=request.form['password']
        sql = "SELECT mobile FROM login WHERE name = %s and password = %s and mobile = %s"
        val = (name,password)
...
def put_vote(name):
    ...
    if request.method=='POST'and name in voterID_array:
        #query to set status to 1 after voting
        sql = "UPDATE voterdetails SET status = 1 WHERE voter_id = %s"  
        val = (name,)
        cursor.execute(sql, val)
        mydb.commit()
